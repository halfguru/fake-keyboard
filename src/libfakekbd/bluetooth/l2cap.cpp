...
auto l2cap_socket::operator=(l2cap_socket&& other) noexcept -> l2cap_socket&
{
  if (this != &other)
  {
    if (fd_ >= 0)
...
auto l2cap_socket::fd() const -> int
{
  return fd_;
}

auto l2cap_socket::is_valid() const -> bool
{
  return fd_ >= 0;
}

auto l2cap_server::listen(bdaddr_t const& addr, uint16_t psm, int backlog) -> hid::Result<void>
{
...
auto l2cap_server::accept() -> hid::Result<std::pair<int, bdaddr_t>>
{
...
auto l2cap_client::connect(bdaddr_t const& src, bdaddr_t const& dst, uint16_t psm) -> hid::Result<void>
{
  fd_ = socket(PF_BLUETOOTH, SOCK_SEQPACKET, BTPROTO_L2CAP);
  if (fd_ < 0)
  {
